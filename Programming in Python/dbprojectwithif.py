...
class StudentList:
    ...
    def edit(self, index, newInfo, toEdit):
        ...
        if (toEdit == "name"): self.students[index].editName(newInfo)
        elif(toEdit == "address"): self.students[index].editAddress(newInfo)
        elif(toEdit =="section"): self.students[index].editClass(newInfo)
        elif(toEdit == "math"): self.students[index].editGrade(newInfo, 0)
        elif(toEdit == "science"): self.students[index].editGrade(newInfo, 1)
        elif(toEdit == "english"): self.students[index].editGrade(newInfo, 2) 
        elif(toEdit == "dutch"): self.students[index].editGrade(newInfo, 3)
        elif(toEdit == "arts"): self.students[index].editGrade(newInfo, 4)
        else: print("Error: Incorrect selection.  Maybe check spelling?")
...
while(runFlag):  
    ...
    try:
        ...
        if(choice == 1): #add new record
        ...
        elif(choice == 5): #sort students by grades
            ...
            if(category == "math"): list.sort(0)
            elif (category == "science"): list.sort(1)
            elif (category == "english"): list.sort(2)
            elif (category == "dutch"): list.sort(3)
            elif (category == "arts"): list.sort(4)
            else: print("Error.  Check the spelling of the subject?")
        
        elif(choice == 6): #save and exit
            runFlag = False
            list.save(filePath)

        elif(choice == 7): #save without exiting
            runFlag = False

        else: #user typed a number not in the menu
